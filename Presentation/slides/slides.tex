...
&  & (key, value) pairs &  & $p$ partitions &  & \\ 
Map & $\rightarrow$ & Clusters: & $\rightarrow$ & $\pi$ partitioning function & $\rightarrow$ & Reduce \\ 
&  & $C(k) = \lbrace (k, v) \rbrace$ &  & $P(j) = \uplus_{\pi(k) = j} C(k)$ &  & \\ 
\end{tabular}
...
\begin{frame}
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{skewtune1}
    \includegraphics[width=0.5\textwidth]{skewtune2}
  \end{figure}
\end{frame}
\begin{frame}{SkewTune~\cite{Kwon:2012:SMS:2213836.2213840}}
...
\begin{frame}{Conclusion}
  ...
  \end{cblock}
  \todo{Perspectives: skew in databases?}
